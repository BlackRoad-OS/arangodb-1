...
static constexpr std::uint64_t kMLMask = 0x0000FFFFFFFFFFFFULL;
...
inline std::uint64_t getLength48(std::uint64_t lo) noexcept {
  return (lo >> 16) & kMLMask;
}
inline std::uint64_t getOrigin8(std::uint64_t lo) noexcept {
  return (lo >> 8) & 0xFFu;
}

static inline uint8_t* allocateSupervised(arangodb::ResourceMonitor& rm,
                                          std::uint64_t len) {
...
void AqlValue::setPointer(uint8_t const* pointer) noexcept {
  ...
  switch (t) {
    case VPACK_SUPERVISED_SLICE: {
      ...
      _data.supervisedSliceMeta.dataPtr = pointer;
      return;
    }
    case VPACK_SUPERVISED_STRING: {
      ...
      _data.supervisedStringMeta.dataPtr = pointer;
      return;
    }
    default:
  ...
  AqlValue AqlValue::at(int64_t position, bool& mustDestroy, bool doCopy)
      const {
    mustDestroy = false;
    auto t = type();
    switch (t) {
  ...
  AqlValue AqlValue::get(CollectionNameResolver const& resolver,
                         std::string_view name, bool& mustDestroy, bool doCopy)
      const {
    mustDestroy = false;
    auto t = type();
    switch (t) {
  ...
  v8::Handle<v8::Value> AqlValue::toV8(
      v8::Isolate * isolate, velocypack::Options const* options) const {
  ...
  void AqlValue::toVelocyPack(velocypack::Options const* options,
                              velocypack::Builder& builder, bool allowUnindexed)
      const {
    auto t = type();
    switch (t) {
      case VPACK_INLINE_INT64:
  ...
  int AqlValue::Compare(velocypack::Options const* options,
                        AqlValue const& left, AqlValue const& right,
                        bool compareUtf8) {
  ...
  AqlValue::AqlValue(VPackSlice slice) {
  ...
  // supervised constructors
  AqlValue::AqlValue(arangodb::ResourceMonitor & rm, velocypack::Slice slice) {
    auto len = static_cast<std::uint64_t>(slice.byteSize());
    if (len <= sizeof(_data.inlineSliceMeta.slice)) {
  ...
  AqlValue::AqlValue(arangodb::ResourceMonitor & rm, velocypack::Slice slice,
                     velocypack::ValueLength length) {
    auto len = static_cast<std::uint64_t>(length);
    if (len <= sizeof(_data.inlineSliceMeta.slice)) {
      initFromSlice(slice, length);
      return;
    }
  ...
  AqlValue::AqlValue(arangodb::ResourceMonitor & rm, uint8_t const* bytes,
    ...
    auto len = static_cast<std::uint64_t>(length);
    if (len <= sizeof(_data.inlineSliceMeta.slice)) {
      initFromSlice(velocypack::Slice(bytes), length);
      return;
    }
    uint8_t* p = allocateSupervised(rm, len);
  ...
  AqlValue::AqlValue(arangodb::ResourceMonitor & rm, AqlValueHintSliceCopy v)
      : AqlValue(rm, v.slice) {}
