...
#include "LoggerOptionsProvider.h"
...
LoggerOptionsProvider::LoggerOptionsProvider(bool threaded)
    : _threaded(threaded) {}

void LoggerOptionsProvider::declareOptions(
  ...
  options
      ->addOption("--log",
                  ...
                  "trace.",
                  new VectorParameter<StringParameter>(&opts.levels),
...
verbose level (e.g. `debug` or `trace`).)");
  ...
  options
      ->addOption("--log.escape-unicode-chars",
                  ...
                  new BooleanParameter(&opts.useUnicodeEscaped))
      .setIntroducedIn(30900)
...
verbose level (e.g. `debug` or `trace`).)");
  ...
  options
      ->addOption("--log.structured-param",
                  ...
                  new VectorParameter<StringParameter>(&opts.structuredLogParams))
...
`/_admin/log/structured` HTTP API.)");
  ...
  options
      ->addOption("--log.output,-o",
                  ...
                  new VectorParameter<StringParameter>(&opts.output))
...
appropriate log topics to the `info` log level.)");
  ...
  options
      ->addOption("--log.max-entry-length",
                  ...
                  new UInt32Parameter(&opts.maxEntryLength))
...
the maximum size of log messages.)");
  ...
  options
      ->addOption("--log.use-local-time",
                  ...
                  new BooleanParameter(&opts.useLocalTime),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
      .setDeprecatedIn(30500)
...
Use `--log.time-format local-datestring` instead.)");
  ...
  options
      ->addOption("--log.use-microtime",
                  ...
                  new BooleanParameter(&opts.useMicrotime),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
      .setDeprecatedIn(30500)
      .setLongDescription(R"(This option is deprecated.
  ...
  options
      ->addOption(
          ...
          new DiscreteValuesParameter<StringParameter>(
              &opts.timeFormatString, LogTimeFormats::getAvailableFormatNames()))
...
`local-datestring`      | 2019-03-28T10:55:23      | local date and time in format YYYY-MM-DDTHH:MM:SS)");
  ...
  options
      ->addOption("--log.ids", "Log unique message IDs.",
                  new BooleanParameter(&opts.showIds))
...
'rocksdb'` (where `144fe` is the log ID).)");
  ...
  options
      ->addOption("--log.role", "Log the server role.",
                  new BooleanParameter(&opts.showRole))
...
- `A`: Agent)");
  ...
  options->addOption(
      ...
      new StringParameter(&opts.fileMode));
  ...
  if (_threaded) {
    ...
    options
        ->addOption("--log.api-enabled",
                    ...
                    new StringParameter(&opts.apiSwitch))
 ...
 - `false`: The `/_admin/log` API is not accessible at all.)");
  ...
  options
      ->addOption("--log.use-json-format",
                  ...
                  new BooleanParameter(&opts.useJson))
...
| `message`  | the actual log message payload)");
  ...
  options->addOption("--log.file-group",
                     ...
                     new StringParameter(&opts.fileGroup));
#endif

  options
      ->addOption("--log.prefix", "Prefix log message with this string.",
                  new StringParameter(&opts.prefix),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
...
`2020-07-23T09:46:03Z --> [17493] INFO ...`)");
  ...
  options->addOption(
      ...
      new BooleanParameter(&opts.lineNumber),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Uncommon));

  options->addOption(
      "--log.shorten-filenames",
      "Shorten filenames in log output (use with `--log.line-number`).",
      new BooleanParameter(&opts.shortenFilenames),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Uncommon));
  ...
  options
      ->addOption("--log.hostname",
                  ...
                  new StringParameter(&opts.hostname))
      .setIntroducedIn(30800)
...
If you set this option to `auto`, the hostname is automatically determined.)");
  ...
  options
      ->addOption("--log.process",
                  ...
                  new BooleanParameter(&opts.processId),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
      .setIntroducedIn(30800);
  ...
  if (_threaded) {
    ...
    options->addOption("--log.keep-logrotate",
                       ...
                       new BooleanParameter(&opts.keepLogRotate),
                       arangodb::options::makeDefaultFlags(
                           arangodb::options::Flags::Uncommon));
  }
  ...
  options->addOption(
      ...
      new BooleanParameter(&opts.foregroundTty),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Uncommon,
  ...
  options
      ->addOption("--log.force-direct",
                  ...
                  new BooleanParameter(&opts.forceDirect),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
...
off to an extra logging thread, which asynchronously writes the log messages.)");
  ...
  options
      ->addOption(
          ...
          new UInt32Parameter(&opts.maxQueuedLogMessages),
          arangodb::options::makeDefaultFlags(
              arangodb::options::Flags::Uncommon))
...
full, log entries are written synchronously until the queue has space again.)");
  ...
  options->addOption(
      ...
      new BooleanParameter(&opts.logRequestParameters),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Uncommon));
...
void LoggerOptionsProvider::validateOptions(
  ...
  if (options->processingResult().touched("log.file")) {
    ...
    if (opts.file == "+" || opts.file == "-") {
      definition = opts.file;
    } else {
      definition = "file://" + opts.file;
    }

    opts.output.push_back(definition);
  }

  if (opts.performance) {
    opts.levels.push_back("performance=trace");
  }
  ...
  if (opts.apiSwitch == "true" || opts.apiSwitch == "on" || opts.apiSwitch == "On") {
    opts.apiEnabled = true;
    opts.apiSwitch = "true";
  } else if (opts.apiSwitch == "jwt" || opts.apiSwitch == "JWT") {
    opts.apiEnabled = true;
    opts.apiSwitch = "jwt";
  } else {
    opts.apiEnabled = false;
    opts.apiSwitch = "false";
  }

  if (!opts.fileMode.empty()) {
    try {
      int result = std::stoi(opts.fileMode, nullptr, 8);
      LogAppenderFileFactory::setFileMode(result);
    } catch (...) {
      LOG_TOPIC("797c2", FATAL, arangodb::Logger::FIXME)
          << "expecting an octal number for log.file-mode, got '" << opts.fileMode
          << "'";
      FATAL_ERROR_EXIT();
    }
  }
...
#ifdef ARANGODB_HAVE_SETGID
  if (!opts.fileGroup.empty()) {
    bool valid = false;
    int gidNumber = NumberUtils::atoi_positive<int>(
        opts.fileGroup.data(), opts.fileGroup.data() + opts.fileGroup.size(), valid);

    if (valid && gidNumber >= 0) {
#ifdef ARANGODB_HAVE_GETGRGID
      std::optional<gid_t> gid = basics::FileUtils::findGroup(opts.fileGroup);
      if (!gid) {
        LOG_TOPIC("174c2", FATAL, arangodb::Logger::FIXME)
            << "unknown numeric gid '" << opts.fileGroup << "'";
        FATAL_ERROR_EXIT();
      }
#endif
    } else {
      ...
      if (gid) {
        ...
        LOG_TOPIC("11a2c", FATAL, arangodb::Logger::FIXME)
            << "cannot convert groupname '" << opts.fileGroup
            << "' to numeric gid: " << TRI_last_error();
        FATAL_ERROR_EXIT();
      }
#else
      LOG_TOPIC("1c96f", FATAL, arangodb::Logger::FIXME)
          << "cannot convert groupname '" << opts.fileGroup << "' to numeric gid";
      FATAL_ERROR_EXIT();
#endif
    }
  ...
  for (auto& output : opts.output) {
    output = basics::StringUtils::replace(output, "$PID",
