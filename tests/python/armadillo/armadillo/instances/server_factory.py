...
class StandardServerFactory:
    ...
    def __init__(self, config_provider: ConfigProvider, logger: Logger, port_allocator: PortAllocator) -> None:
    ...
    def create_server_instances(self, servers_config: list[ServerConfig]) -> Dict[str, ArangoServer]:
        ...
        for i, server_config in enumerate(servers_config):
            ...
            self._logger.debug('Created server instance %s with role %s on port %s', server_id, server_config.role.value, server_config.port)
        return servers
    ...
    def _create_command_builder(self) -> ServerCommandBuilder:
        ...
        return ServerCommandBuilder(config_provider=self._config_provider, logger=self._logger)

    def _create_health_checker(self) -> ServerHealthChecker:
        """Create health checker with injected dependencies."""
        return ServerHealthChecker(logger=self._logger, auth_provider=self._auth_provider)
