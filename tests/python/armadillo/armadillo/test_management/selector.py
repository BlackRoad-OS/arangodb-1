...
class FilterCriteria:
    ...
    def __post_init__(self):
        ...
        if (
            self.filter_type in [FilterType.PATTERN, FilterType.PATH]
...
class Filter:
    ...
    def matches(self, test_item: PytestItem) -> bool:
        ...
        try:
            if self.criteria.filter_type == FilterType.MARKER:
                ...
                self.logger.warning(
                    "Unknown filter type: %s", self.criteria.filter_type
                )
                return False
    ...
    def _matches_pattern(self, test_item: PytestItem) -> bool:
        ...
        return (
            self.criteria.pattern.search(test_name) is not None
            or self.criteria.pattern.search(test_nodeid) is not None
        )
    ...
    def _matches_tag(self, test_item: PytestItem) -> bool:
        ...
        for marker in test_item.iter_markers():
            ...
            if hasattr(marker, "kwargs") and "tags" in marker.kwargs:
                ...
                if (
                    isinstance(marker_tags, (list, set))
                    and self.criteria.value in marker_tags
                ):
                    return True
        return False
...
class Selector:
    ...
    def add_pattern_filter(
        self, pattern: str, operation: FilterOperation
    ) -> "Selector":
    ...
    def add_path_filter(
        self, path_pattern: str, operation: FilterOperation
    ) -> "Selector":
    ...
    def add_custom_filter(
        self,
        filter_func: Callable[[PytestItem], bool],
    ...
    def select_tests(self, collected_tests: List[PytestItem]) -> SelectionResult:
        ...
        """
        self.logger.info(
            "Selecting tests from %s collected tests", len(collected_tests)
        )
        ...
        self.logger.info(
            "Test selection complete: %s/%s selected (%s%%)",
    ...
    def _should_include_test(
        self, test_item: PytestItem, filter_stats: Dict[str, Dict[str, int]]
    ) -> bool:
