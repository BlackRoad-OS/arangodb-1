...
@analyze_app.command()
def summary(result_files: List[str]=typer.Argument(help='JSON result files to analyze'), output_format: str=typer.Option('rich', '--format', help='Output format: rich, plain, json')):
...
def list_analyzers():
    ...
    analyzers = [('pretty', 'Human-readable summary with failure details', '✅ Phase 1'), ('table', 'Column-oriented statistics table', '✅ Phase 1'), ('long-running', 'Identify slowest tests', '⏳ Phase 2'), ('short-lifetime', 'Detect inefficient setup/teardown ratios', '⏳ Phase 2'), ('resources', 'Process and memory usage analysis', '⏳ Phase 2'), ('leaks', 'SUT checker violation summary', '⏳ Phase 3'), ('crash-summary', 'Crash and sanitizer report analysis', '⏳ Phase 4'), ('perf-regression', 'Performance regression detection', '⏳ Phase 5')]
...
def _load_results(result_files: List[Path]) -> Dict[str, Any]:
    ...
    aggregated = {'total_files': len(result_files), 'total_tests': 0, 'total_duration': 0.0, 'summary': {'total': 0, 'passed': 0, 'failed': 0, 'skipped': 0, 'error': 0, 'timeout': 0, 'crashed': 0}, 'files': [], 'failed_tests': []}
    for result_file in result_files:
        ...
        try:
            ...
            file_info = {'file': str(result_file), 'duration': result_data.get('duration_s', 0.0), 'test_count': len(result_data.get('tests', {})), 'summary': result_data.get('meta', {}).get('summary', {})}
            ...
            for test_name, test_data in tests.items():
                if test_data.get('status') in ['failed', 'error', 'crashed', 'timeout']:
                    aggregated['failed_tests'].append({'name': test_name, 'status': test_data.get('status'), 'duration': test_data.get('duration_s', 0.0), 'message': test_data.get('error_message') or test_data.get('failure_message'), 'file': str(result_file)})
        except Exception as e:
...
def _display_rich_summary(results: Dict[str, Any]) -> None:
    ...
    if total > 0:
        ...
        console.print(Panel(f"[{color}]Success Rate: {success_rate:.1f}% ({summary['passed']}/{total})[/{color}]", title='Overall Result'))
    if results['failed_tests']:
        ...
        for test in results['failed_tests'][:10]:
            message = test.get('message', '')[:100] if test.get('message') else 'No message'
            failed_table.add_row(test['name'], test['status'], f"{test['duration']:.2f}s", message)
        if len(results['failed_tests']) > 10:
            failed_table.add_row(f"... and {len(results['failed_tests']) - 10} more", '', '', '')
