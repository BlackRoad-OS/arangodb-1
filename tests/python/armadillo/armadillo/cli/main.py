...
app = typer.Typer(name='armadillo', help='Modern ArangoDB Testing Framework', no_args_is_help=True, rich_markup_mode='markdown')
...
@app.callback()
def main(verbose: int=typer.Option(0, '--verbose', '-v', count=True, help='Increase verbosity level'), config_file: Optional[Path]=typer.Option(None, '--config', '-c', help='Configuration file path'), build_dir: Optional[Path]=typer.Option(None, '--build-dir', '-b', help='ArangoDB build directory (auto-detected if not specified)')):
