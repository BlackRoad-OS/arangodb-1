...
class ResourceTracker:
    ...
    def register_resource_type(
        self, resource_type: str, cleanup_callback: callable
    ) -> None:
    ...
    def track_resource(self, resource_type: str, resource: Any) -> None:
        ...
        with self._lock:
            ...
            logger.debug(
                "ResourceTracker[%s]: Tracking %s resource: %s",
                self._name,
                resource_type,
                resource,
            )
    ...
    def untrack_resource(self, resource_type: str, resource: Any) -> None:
        ...
        with self._lock:
            if resource_type in self._allocated_resources:
                ...
                logger.debug(
                    "ResourceTracker[%s]: Untracking %s resource: %s",
                    self._name,
                    resource_type,
                    resource,
                )
            ...
            for resource_type, resources in self._allocated_resources.items():
                if resources and resource_type in self._cleanup_callbacks:
                    try:
                        ...
                        logger.info(
                            "ResourceTracker[%s]: Cleaning up %s %s resources",
                            self._name,
                            len(resources_list),
                            resource_type,
                        )
                        ...
                        logger.error(
                            "ResourceTracker[%s]: Failed to cleanup %s resources: %s",
                            self._name,
                            resource_type,
                            e,
                        )
    ...
    def cleanup_type(self, resource_type: str) -> None:
        ...
        with self._lock:
            if (
                resource_type in self._allocated_resources
                and resource_type in self._cleanup_callbacks
            ):
...
class PortPool:
    ...
    def acquire_multiple(self, count: int) -> List[int]:
        ...
        for port in ports:
            self._resource_tracker.track_resource("ports", port)
        logger.debug(
            "PortPool[%s]: Acquired %s ports: %s", self._name, len(ports), ports
        )
        return ports
    ...
    def release_multiple(self, ports: List[int]) -> None:
        ...
        for port in ports:
            self._resource_tracker.untrack_resource("ports", port)
        logger.debug(
            "PortPool[%s]: Released %s ports: %s", self._name, len(ports), ports
        )
    ...
    def _cleanup_ports(self, ports: List[int]) -> None:
        ...
        if ports:
            logger.info(
                "PortPool[%s]: Cleaning up %s allocated ports: %s",
                self._name,
                len(ports),
                ports,
            )
...
class ManagedPortPool(PortPool):
    ...
    def __init__(
        ...
        resource_tracker = ResourceTracker(
            f"{name}_managed_ports" if name else "managed_ports"
        )
        super().__init__(
        ...
        logger.info(
            "Created ManagedPortPool[%s] with base_port=%s, max_ports=%s",
            self._name,
            base_port,
            max_ports,
        )
...
def create_ephemeral_port_pool(
    name: str = "", base_port: int = 8529, max_ports: int = 1000
) -> ManagedPortPool:
