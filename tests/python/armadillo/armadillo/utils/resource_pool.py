...
class ResourceTracker:
    ...
    def track_resource(self, resource_type: str, resource: Any) -> None:
        ...
        with self._lock:
            ...
            logger.debug('ResourceTracker[%s]: Tracking %s resource: %s', self._name, resource_type, resource)
    ...
    def untrack_resource(self, resource_type: str, resource: Any) -> None:
        ...
        with self._lock:
            if resource_type in self._allocated_resources:
                ...
                logger.debug('ResourceTracker[%s]: Untracking %s resource: %s', self._name, resource_type, resource)
...
class PortPool:
    ...
    def __init__(self, port_manager: Optional[PortManager]=None, resource_tracker: Optional[ResourceTracker]=None, name: str='') -> None:
        ...
        logger.debug('Created PortPool[%s]', self._name)
    ...
    def acquire(self, preferred: Optional[int]=None) -> int:
        ...
        logger.debug('PortPool[%s]: Acquired port %s', self._name, port)
        return port
    ...
    def acquire_multiple(self, count: int) -> List[int]:
        ...
        for port in ports:
            self._resource_tracker.track_resource('ports', port)
        logger.debug('PortPool[%s]: Acquired %s ports: %s', self._name, len(ports), ports)
        return ports
    ...
    def release(self, port: int) -> None:
        ...
        logger.debug('PortPool[%s]: Released port %s', self._name, port)
    ...
    def release_multiple(self, ports: List[int]) -> None:
        ...
        for port in ports:
            self._resource_tracker.untrack_resource('ports', port)
        logger.debug('PortPool[%s]: Released %s ports: %s', self._name, len(ports), ports)
    ...
    def shutdown(self) -> None:
        ...
        logger.info('PortPool[%s]: Shutting down', self._name)
        self._resource_tracker.cleanup_all()

    def _cleanup_ports(self, ports: List[int]) -> None:
        """Cleanup callback for ports."""
        if ports:
            logger.info('PortPool[%s]: Cleaning up %s allocated ports: %s', self._name, len(ports), ports)
            self._port_manager.release_ports(ports)

class ManagedPortPool(PortPool):
    """Port pool with enhanced resource management and isolation."""

    def __init__(self, base_port: int=8529, max_ports: int=1000, name: str='', enable_persistence: bool=True, work_dir: Optional[Path]=None) -> None:
        ...
        super().__init__(port_manager=port_manager, resource_tracker=resource_tracker, name=name or 'managed')
        atexit.register(self.shutdown)
        logger.info('Created ManagedPortPool[%s] with base_port=%s, max_ports=%s', self._name, base_port, max_ports)

def create_isolated_port_pool(name: str, base_port: int=8529, max_ports: int=1000, work_dir: Optional[Path]=None) -> ManagedPortPool:
