...
class AuthProvider:
    ...
    def create_user_token(
        ...
        logger.debug(
            "Created user token for %s with %s permissions", username, len(permissions)
        )
        return token

    def create_service_token(
        self, service_name: str, permissions: List[str], ttl: float = 86400.0
    ) -> str:
    ...
    def create_cluster_auth_headers(
        self, cluster_id: str, role: str = "test_cluster", ttl: float = 7200.0
    ) -> Dict[str, str]:
    ...
    def get_active_tokens(self) -> Dict[str, Dict[str, Any]]:
        ...
        expired_tokens = [
            token_id
            for token_id, info in self._active_tokens.items()
