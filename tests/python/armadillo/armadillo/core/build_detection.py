...
class BuildDetector:
    ...
    def detect_build_directory(self, search_from: Optional[Path]=None) -> Optional[Path]:
        ...
        search_roots = [search_from, search_from.parent, search_from.parent.parent, search_from.parent.parent.parent]
        for root in search_roots:
            if root.exists():
                found_path = self._filesystem_search(root)
                if found_path:
                    logger.warning('Using guessed arangod location: %s', found_path / 'arangod')
                    return found_path
