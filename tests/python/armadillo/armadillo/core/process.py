        ...
        try:
            with timeout_scope(effective_timeout, f'process_exec_{command[0]}'):
                ...
                try:
                    ...
                    result = ProcessResult(returncode=process.returncode, stdout=stdout or '', stderr=stderr or '', duration=duration)
...
class ProcessSupervisor:
    ...
    def start(self,
        ...
        log_process_event(logger, 'supervisor.start', process_id=process_id, command=command)
        ...
        try:
            ...
            log_process_event(logger, 'supervisor.started', process_id=process_id, pid=process.pid)
            if use_streaming:
            ...
            try:
                ...
                pass
            log_process_event(logger, 'supervisor.start_failed', process_id=process_id, error=str(e))
            self._cleanup_process(process_id)
            raise ProcessStartupError(f'Failed to start process {process_id}: {e}{error_output}') from e

    def stop(self, process_id: str, graceful: bool=True, timeout: float=30.0) -> None:
        ...
        log_process_event(logger, 'supervisor.stop', process_id=process_id, graceful=graceful, timeout=timeout)
        try:
            if graceful:
                logger.debug('Sending SIGTERM to process group %s (PGID: -%s, timeout: %ss)', process_id, process.pid, timeout)
                try:
                    if os.name != 'nt':
                    ...
                    logger.debug('Could not send SIGTERM to process group %s: %s', process.pid, e)
                    try:
                        process.terminate()
                    except:
                        pass
                try:
                    ...
                    log_process_event(logger, 'supervisor.stopped', process_id=process_id, method='graceful_sigterm_group')
                    logger.debug('Process group %s terminated gracefully via SIGTERM', process_id)
                    return
                except subprocess.TimeoutExpired:
                    logger.warning('Process group %s (PGID: %s) did not respond to SIGTERM within %ss, escalating to SIGKILL', process_id, process.pid, timeout)
                    log_process_event(logger, 'supervisor.graceful_timeout', process_id=process_id, timeout=timeout)
            logger.debug('Sending SIGKILL to process group %s (PGID: -%s)', process_id, process.pid)
            try:
                if os.name != 'nt':
            ...
            try:
                ...
                log_process_event(logger, 'supervisor.stopped', process_id=process_id, method='killed_sigkill')
                logger.debug('Process %s killed successfully via SIGKILL', process_id)
            except subprocess.TimeoutExpired:
                logger.error('CRITICAL: Process group %s (PID: %s) did not die even after SIGKILL!', process_id, process.pid)
                log_process_event(logger, 'supervisor.sigkill_timeout', process_id=process_id, pid=process.pid)
                logger.warning('Attempting emergency process tree kill for %s (PID: %s)', process_id, process.pid)
                try:
    ...
    def is_running(self, process_id: str) -> bool:
        ...
        if not is_running:
            logger.debug('is_running(%s): process exited with code %s', process_id, poll_result)
        return is_running
    ...
    def _wait_for_readiness(self, process_id: str, readiness_check: Callable[[], bool], timeout: float) -> None:
    ...
    def _start_monitoring(self, process_id: str) -> None:
        ...
        monitor_thread = threading.Thread(target=self._monitor_process, args=(process_id,), name=f'ProcessMonitor-{process_id}', daemon=True)
    ...
    def _handle_process_exit(self, process_id: str, exit_code: int) -> None:
        ...
        process_info = self._process_info.get(process_id)
        if exit_code == 0:
            log_process_event(logger, 'supervisor.exited', process_id=process_id, exit_code=exit_code)
        else:
            log_process_event(logger, 'supervisor.crashed', process_id=process_id, exit_code=exit_code)
            process = self._processes.get(process_id)
            stderr_output = ''
...
def kill_process_tree(root_pid: int, signal_num: int=signal.SIGKILL, timeout: float=5.0) -> bool:
    ...
    """
    logger.debug('Killing process tree rooted at PID %s with signal %s', root_pid, signal_num)
    try:
...
def kill_all_supervised_processes() -> None:
    ...
    try:
        if hasattr(_process_supervisor, '_processes'):
            ...
            if processes:
                logger.warning('Emergency killing %s supervised processes and their children', len(processes))
                ...
                if failed_trees:
                    logger.warning('Phase 2: Force killing %s stubborn process trees', len(failed_trees))
