...
class ResultCollector:
    ...
    def finalize_results(self) -> SuiteExecutionResults:
        ...
        self.metadata.update({'start_time': datetime.fromtimestamp(self.start_time, tz=timezone.utc).isoformat(), 'end_time': datetime.fromtimestamp(end_time, tz=timezone.utc).isoformat(), 'total_duration': total_duration})
        results = SuiteExecutionResults(tests=self.tests.copy(), total_duration=total_duration, summary=summary, metadata=self.metadata.copy())
        self._finalized = True
        logger.info('Finalized results: %s tests, %ss total', len(self.tests), total_duration)
        return results
    ...
    def export_results(self, formats: List[str], output_dir: Path) -> Dict[str, Path]:
        ...
        if not self._finalized:
            ...
            results = SuiteExecutionResults(tests=self.tests, total_duration=time.time() - self.start_time, summary=self._calculate_summary(), metadata=self.metadata)
    ...
    def _calculate_summary(self) -> Dict[str, int]:
        ...
        summary = {'total': len(self.tests), 'passed': 0, 'failed': 0, 'skipped': 0, 'error': 0, 'timeout': 0, 'crashed': 0}
    ...
    def _export_json(self, results: SuiteExecutionResults, file_path: Path) -> None:
        ...
        export_data = {'framework_version': '1.0.0', 'timestamp_utc': results.metadata.get('start_time'), 'duration_s': results.total_duration, 'tests': {}, 'meta': {'summary': results.summary, 'metadata': results.metadata}}
        for test in results.tests:
            test_data = {'status': test.outcome.value, 'duration_s': test.duration, 'setup_duration_s': test.setup_duration, 'teardown_duration_s': test.teardown_duration}
