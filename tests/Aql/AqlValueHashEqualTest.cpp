...
/**
 * IMPORTANT: Test Strategy for Hash/Comparison Tests
 * 
 * These tests document the EXPECTED behavior of AqlValue hash and equality,
 * based on how they are used in production code (toVelocyPack, cloneToBlock).
 * 
 * CURRENT IMPLEMENTATION (OLD HASH - arangodb_devel_4):
 * - Supervised slices: Hash by pointer address, compare by content
 * - Violates hash/equality contract: same content → equal, but different hashes
 * - Some tests will FAIL (expected) - this documents the bug
 * 
 * EXPECTED IMPLEMENTATION (NEW HASH - arangodb_devel_3):
 * - Supervised slices: Hash by content, compare by content
 * - Satisfies hash/equality contract: same content → equal → same hash
 * - All tests will PASS - this verifies the fix
 * 
 * PURPOSE:
 * - Tests serve as SPECIFICATIONS of correct behavior
 * - Failures with old hash PROVE the bug exists
 * - Passes with new hash PROVE the fix works
 * 
 * See HASH_COMPARISON_TEST_STRATEGY.md for detailed explanation.
 */

namespace {

// Helper functions to create AqlValue instances (matching AqlValueCompare.cpp pattern)
...
TEST_F(AqlValueHashEqualTest, hash_equal_consistency_different_storage_types) {
  ...
  size_t h1 = hasher(val1);
  size_t h2 = hasher(val2);
  EXPECT_EQ(h1, h2) << "Same semantic value in different storage must have same hash";
  EXPECT_TRUE(equal(val1, val2)) << "Same semantic value in different storage must be equal";
}

TEST_F(AqlValueHashEqualTest, hash_equal_consistency_different_values) {
  // Different values should ideally have different hashes (but collisions are possible)
  AqlValue val1(makeAQLValue(int64_t{42}));
  AqlValue val2(makeAQLValue(int64_t{43}));
  
  EXPECT_FALSE(equal(val1, val2)) << "Different values should not be equal";
  // Note: We can't guarantee different hashes due to collisions, but we can test
  // that if hashes are different, values are different
  size_t h1 = hasher(val1);
  size_t h2 = hasher(val2);
  if (h1 != h2) {
    EXPECT_FALSE(equal(val1, val2)) << "Different hashes imply different values";
  }
}
  ...
  for (size_t i = 0; i < testValues.size(); ++i) {
    for (size_t j = 0; j < testValues.size(); ++j) {
      ...
      EXPECT_EQ(shouldEqual, equal(val1, val2))
          << "Values " << testValues[i] << " and " << testValues[j] << " comparison failed";
...
TEST_F(AqlValueHashEqualTest, inline_double_zero_variants) {
  // +0.0 and -0.0 should be equal (normalized comparison)
  AqlValue val1(makeAQLValue(0.0));
  AqlValue val2(makeAQLValue(-0.0));
  
  // Note: normalizedHash should treat them the same
  EXPECT_TRUE(equal(val1, val2)) << "+0.0 and -0.0 should be equal";
  EXPECT_EQ(hasher(val1), hasher(val2)) << "+0.0 and -0.0 should have same hash";
}
...
TEST_F(AqlValueHashEqualTest, number_semantic_equality_int64_vs_vpack) {
  ...
  EXPECT_TRUE(equal(val1, val2)) << "Same number in different storage should be equal";
  EXPECT_EQ(hasher(val1), hasher(val2)) << "Same number in different storage should have same hash";
}
...
TEST_F(AqlValueHashEqualTest, number_semantic_equality_uint64_vs_vpack) {
  ...
  EXPECT_TRUE(equal(val1, val2)) << "Same large number in different storage should be equal";
  EXPECT_EQ(hasher(val1), hasher(val2)) << "Same large number in different storage should have same hash";
}
...
TEST_F(AqlValueHashEqualTest, number_semantic_equality_double_vs_vpack) {
  ...
  EXPECT_TRUE(equal(val1, val2)) << "Same double in different storage should be equal";
  EXPECT_EQ(hasher(val1), hasher(val2)) << "Same double in different storage should have same hash";
}
...
TEST_F(AqlValueHashEqualTest, number_semantic_equality_int_vs_double) {
  ...
  // Note: normalizedHash should treat them the same
  EXPECT_TRUE(equal(val1, val2)) << "Integer 42 and double 42.0 should be equal";
  EXPECT_EQ(hasher(val1), hasher(val2)) << "Integer 42 and double 42.0 should have same hash";
}
...
TEST_F(AqlValueHashEqualTest, cross_type_different_slice_types) {
  ...
  EXPECT_TRUE(equal(inlineVal, sliceVal)) << "Same string value should be equal regardless of storage";
  EXPECT_EQ(hasher(inlineVal), hasher(sliceVal)) << "Same string value should have same hash";
}
...
TEST_F(AqlValueHashEqualTest, unordered_set_deduplication) {
  ...
  EXPECT_FALSE(result2.second) << "Same semantic value in different storage should not be inserted";
  EXPECT_EQ(1U, set.size()) << "Set should still have 1 element";
}
...
TEST_F(AqlValueHashEqualTest, unordered_map_key_lookup) {
  ...
  EXPECT_EQ(100, map[key3]) << "Same semantic value in different storage should find same map entry";
...
TEST_F(AqlValueHashEqualTest, stress_test_many_values) {
  ...
  EXPECT_GT(hashes.size(), 100U) << "Hash function should provide good distribution";
}
...
TEST_F(AqlValueHashEqualTest, real_world_deduplication_scenario) {
  // Simulate the InputAqlItemRow::cloneToBlock scenario
  std::unordered_set<AqlValue> cache;
  
  // Create same value multiple times with potentially different storage
  AqlValue original(makeAQLValue(int64_t{42}));
  
  // Clone it (simulating the scenario)
  VPackBuilder builder;
  builder.add(VPackValue(42));
  AqlValue cloned(builder.slice());
  
  cache.insert(original);
  EXPECT_EQ(1U, cache.size());
  
  // Cloned value should be found in cache (same semantic value)
  auto it = cache.find(cloned);
  EXPECT_NE(cache.end(), it) << "Cloned value should be found in cache";
  EXPECT_TRUE(equal(original, *it)) << "Found value should equal original";
}

TEST_F(AqlValueHashEqualTest, real_world_serialization_scenario) {
  // Simulate the AqlItemBlock::toVelocyPack scenario
  ...
  // Later, same value comes in different storage type
  VPackBuilder builder;
  builder.add(VPackValue(42));
  AqlValue val2(builder.slice());
  
  // Should find existing entry
  auto it = table.find(val2);
  EXPECT_NE(table.end(), it) << "Same semantic value should be found";
  EXPECT_EQ(100, it->second) << "Should get same position";
}
...
TEST_F(AqlValueCloneToBlockTest, clone_to_block_deduplicates_managed_slices) {
  // Test that cloneToBlock properly deduplicates AqlValues that require destruction
  // This simulates the InputAqlItemRow::cloneToBlock scenario
  ...
  sourceBlock->setValue(0, 1, managedVal2);  // Same value, should be deduplicated
  sourceBlock->setValue(0, 2, AqlValue(std::string(200, 'y')));  // Different value
  ...
  EXPECT_EQ(hasher(val0), hasher(val1)) << "Deduplicated values should have same hash";
...
TEST_F(AqlValueCloneToBlockTest, clone_to_block_handles_multiple_storage_types) {
  // Test that cloneToBlock handles values with different storage types but same content
  ...
  sourceBlock->setValue(0, 3, AqlValue(strVal));  // Same string, different object
  
  InputAqlItemRow sourceRow(sourceBlock, 0);
  
  RegIdFlatSet registers = {RegisterId{0}, RegisterId{1}, RegisterId{2}, RegisterId{3}};
  ...
  AqlValue const& val0 = clonedBlock->getValueReference(0, 0);
  AqlValue const& val1 = clonedBlock->getValueReference(0, 1);
  EXPECT_TRUE(equal(val0, val1)) << "Same number in different storage should be equal";
...
TEST_F(AqlValueCloneToBlockTest, clone_to_block_handles_empty_and_none) {
  ...
  RegIdFlatSet registers = {RegisterId{0}, RegisterId{1}, RegisterId{2}, RegisterId{3}, RegisterId{4}};
  auto clonedBlock = sourceRow.cloneToBlock(itemBlockManager, registers, 5);
  
  EXPECT_NE(nullptr, clonedBlock.get());
  
  // Empty values don't require destruction, so they're not deduplicated via cache
...
TEST_F(AqlValueCloneToBlockTest, clone_to_block_transfers_between_blocks) {
  // Test transferring values from one block to another (simulating real query execution)
  ...
  auto cloned1 = row1.cloneToBlock(itemBlockManager, regs, 1);
  
  // Clone from second block
  InputAqlItemRow row2(sourceBlock2, 0);
  auto cloned2 = row2.cloneToBlock(itemBlockManager, regs, 1);
  ...
  EXPECT_TRUE(equal(val1, val2)) << "Same content from different blocks should be equal";
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_deduplicates_values) {
  // Simulate AqlItemBlock::toVelocyPack serialization with deduplication
  ...
  // Use FlatHashMap like in toVelocyPack
  containers::FlatHashMap<AqlValue, size_t> table;
  size_t pos = 2;
  
  // Simulate the serialization loop
  ...
  // Should have only 2 unique values (42 and 43)
  EXPECT_EQ(2U, table.size()) << "Should deduplicate to 2 unique values";
  
  // Verify hash consistency
  AqlValue testVal = makeAQLValue(int64_t{42});
  auto it = table.find(testVal);
  EXPECT_NE(table.end(), it) << "Should find value 42 in table";
  
  // Verify different storage types are treated as same
  VPackBuilder builder;
  builder.add(VPackValue(42));
  AqlValue testVal2(builder.slice());
  auto it2 = table.find(testVal2);
  EXPECT_NE(table.end(), it2) << "Same value in different storage should be found";
  EXPECT_EQ(it->second, it2->second) << "Should map to same position";
}
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_handles_all_storage_types) {
  ...
  EXPECT_EQ(6U, table.size()) << "Should have 6 unique non-range values";
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_semantic_equality_across_types) {
  // Test that same semantic value in different storage types maps to same position
  ...
  EXPECT_EQ(it1->second, it2->second) << "Same number in different storage should map to same position";
  EXPECT_EQ(it2->second, it3->second) << "Same number in different storage should map to same position";
}
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_handles_arrays_and_objects) {
  ...
  EXPECT_NE(table.end(), it1);
  EXPECT_NE(table.end(), it2);
  EXPECT_EQ(it1->second, it2->second) << "Same arrays should map to same position";
}
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_stress_test_many_duplicates) {
  ...
  for (const auto& val : uniqueVals) {
    EXPECT_NE(table.end(), table.find(val)) << "All unique values should be in table";
  }
}
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_memory_safety_multiple_blocks) {
  ...
  EXPECT_NE(table1.end(), it) << "Should still find value after multiple operations";
}

// ============================================================================
// REAL USAGE SCENARIO TESTS - These test the actual behavior in production code
// ============================================================================

/**
 * Test class for testing AqlItemBlock::toVelocyPack() deduplication behavior.
 * This tests the actual usage of hash/comparison in production code.
 */
class AqlValueToVelocyPackDeduplicationTest : public ::testing::Test {
 protected:
  void SetUp() override {
    monitor = std::make_unique<arangodb::ResourceMonitor>(
        arangodb::GlobalResourceMonitor::instance());
    itemBlockManager = std::make_unique<AqlItemBlockManager>(
        *monitor);
  }

  void TearDown() override {
    itemBlockManager.reset();
    monitor.reset();
  }

  std::unique_ptr<arangodb::ResourceMonitor> monitor;
  std::unique_ptr<AqlItemBlockManager> itemBlockManager;
};

/**
 * CRITICAL TEST: toVelocyPack deduplication with supervised slices.
 * 
 * EXPECTED BEHAVIOR:
 * - When serializing an AqlItemBlock with multiple rows containing supervised slices
 *   that have the SAME CONTENT but DIFFERENT POINTERS, they should be deduplicated.
 * - The hash map in toVelocyPack() should recognize them as the same value and
 *   only serialize the content once, referencing it by position.
 * 
 * CURRENT IMPLEMENTATION (OLD HASH - BUGGY):
 * - Hash uses pointer address → different supervised slices with same content have
 *   different hashes → NOT deduplicated → wastes space
 * 
 * EXPECTED WITH NEW HASH:
 * - Hash uses content → same supervised slices with same content have same hash
 *   → deduplicated → saves space
 */
TEST_F(AqlValueToVelocyPackDeduplicationTest, 
       toVelocyPack_DeduplicatesSupervisedSlices_ByContent) {
  // Create a block with multiple rows containing supervised slices with SAME CONTENT
  constexpr size_t numRows = 5;
  constexpr RegisterId::value_t numCols = 1;
  auto block = itemBlockManager->requestBlock(numRows, numCols);
  
  std::string content = "same supervised content for all rows";
  
  // Create supervised slices with SAME CONTENT but DIFFERENT POINTERS
  // Each call to AqlValue(content) creates a new allocation
  std::vector<AqlValue> supervisedSlices;
  for (size_t i = 0; i < numRows; ++i) {
    supervisedSlices.emplace_back(content);
    block->setValue(i, 0, supervisedSlices.back());
  }
  
  // Verify they have different pointers (different allocations)
  for (size_t i = 1; i < numRows; ++i) {
    EXPECT_NE(supervisedSlices[0].data(), supervisedSlices[i].data())
  ...
  std::equal_to<AqlValue> equal;
  for (size_t i = 1; i < numRows; ++i) {
    EXPECT_TRUE(equal(supervisedSlices[0], supervisedSlices[i]))
        << "Supervised slices with same content should be equal";
  }
  
  // Simulate the deduplication logic from toVelocyPack()
  // This is what happens in AqlItemBlock::toVelocyPack() at line 761
  containers::FlatHashMap<AqlValue, size_t> table;
  size_t pos = 2;  // Position in "raw" array (starts at 2)
  ...
  for (size_t row = 0; row < numRows; ++row) {
    ...
    if (inserted) {
      // New unique value - would serialize it here
      uniqueCount++;
      pos++;
    } else {
      // Duplicate found - would reference existing position
      // This is the EXPECTED behavior: same content should be deduplicated
    }
  }
  
  // EXPECTED: Should have only 1 unique value (all rows have same content)
  // CURRENT (OLD HASH): Will have 5 unique values (different pointers)
  // NEW HASH: Will have 1 unique value (same content)
  EXPECT_EQ(1U, uniqueCount)
      << "Expected: Same content should deduplicate to 1 unique value. "
      << "Current (old hash): Different pointers prevent deduplication. "
      << "This test documents the EXPECTED behavior - it may fail with old hash.";
  EXPECT_EQ(1U, table.size())
      << "Hash map should contain only 1 entry for same content";
  
  // Cleanup
  block.reset(nullptr);
  for (auto& val : supervisedSlices) {
    val.destroy();
  }
}

/**
 * Test cloneToBlock deduplication with supervised slices.
 * 
 * EXPECTED BEHAVIOR:
 * - When cloning a row with multiple registers containing supervised slices
 *   that have the SAME CONTENT but DIFFERENT POINTERS, they should be deduplicated.
 * - The unordered_set cache should recognize them as the same value and only
 *   clone the content once, reusing it across registers.
 * 
 * CURRENT IMPLEMENTATION (OLD HASH - BUGGY):
 * - Hash uses pointer address → different supervised slices with same content have
 *   different hashes → NOT deduplicated → wastes memory
 * 
 * EXPECTED WITH NEW HASH:
 * - Hash uses content → same supervised slices with same content have same hash
 *   → deduplicated → saves memory
 */
TEST_F(AqlValueToVelocyPackDeduplicationTest,
       cloneToBlock_DeduplicatesSupervisedSlices_ByContent) {
  // Create a source block with one row containing multiple registers
  // All registers have supervised slices with SAME CONTENT but DIFFERENT POINTERS
  constexpr RegisterId::value_t numCols = 5;
  auto sourceBlock = itemBlockManager->requestBlock(1, numCols);
  
  std::string content = "same supervised content for all registers";
  
  // Create supervised slices with SAME CONTENT but DIFFERENT POINTERS
  std::vector<AqlValue> supervisedSlices;
  for (RegisterId::value_t col = 0; col < numCols; ++col) {
    supervisedSlices.emplace_back(content);
    sourceBlock->setValue(0, col, supervisedSlices.back());
  }
  
  // Verify they have different pointers
  for (RegisterId::value_t col = 1; col < numCols; ++col) {
    EXPECT_NE(supervisedSlices[0].data(), supervisedSlices[col].data())
        << "Supervised slices should have different pointers";
  }
  ...
  std::equal_to<AqlValue> equal;
  for (RegisterId::value_t col = 1; col < numCols; ++col) {
    EXPECT_TRUE(equal(supervisedSlices[0], supervisedSlices[col]))
        << "Supervised slices with same content should be equal";
  }
  
  // Simulate the deduplication logic from InputAqlItemRow::cloneToBlock()
  // This is what happens at line 54-79
  InputAqlItemRow sourceRow(sourceBlock, 0);
  RegIdFlatSet allRegs;
  for (RegisterId::value_t col = 0; col < numCols; ++col) {
    allRegs.insert(RegisterId::makeRegular(col));
  }
  
  SharedAqlItemBlockPtr clonedBlock = sourceRow.cloneToBlock(
      *itemBlockManager, allRegs, numCols);
  
  // Count how many unique supervised slices were cloned
  // In cloneToBlock(), the cache is used to deduplicate
  // We can verify deduplication by checking if all registers point to same data
  void const* firstData = clonedBlock->getValueReference(0, 0).data();
  size_t uniqueDataPtrs = 1;
  
  for (RegisterId::value_t col = 1; col < numCols; ++col) {
    void const* colData = clonedBlock->getValueReference(0, col).data();
    if (colData != firstData) {
      uniqueDataPtrs++;
    }
  }
  
  // EXPECTED: Should have only 1 unique data pointer (all registers reuse same clone)
  // CURRENT (OLD HASH): Will have 5 unique data pointers (different pointers prevent deduplication)
  // NEW HASH: Will have 1 unique data pointer (same content deduplicated)
  EXPECT_EQ(1U, uniqueDataPtrs)
      << "Expected: Same content should deduplicate to 1 unique clone. "
      << "Current (old hash): Different pointers prevent deduplication. "
      << "This test documents the EXPECTED behavior - it may fail with old hash.";
  
  // Verify all cloned values are equal by content
  for (RegisterId::value_t col = 1; col < numCols; ++col) {
    EXPECT_TRUE(equal(clonedBlock->getValueReference(0, 0),
                     clonedBlock->getValueReference(0, col)))
        << "All cloned values should be equal by content";
  }
...
/**
 * Test that demonstrates the memory waste with old hash.
 * 
 * This test shows the CONSEQUENCE of the bug: without proper deduplication,
 * we waste memory by storing duplicate content multiple times.
 */
TEST_F(AqlValueToVelocyPackDeduplicationTest,
       toVelocyPack_MemoryWaste_WithoutDeduplication) {
  constexpr size_t numRows = 10;
  constexpr RegisterId::value_t numCols = 1;
  auto block = itemBlockManager->requestBlock(numRows, numCols);
  
  std::string content = "large supervised content that should be deduplicated";
  size_t contentSize = content.size();
  
  // Create supervised slices with SAME CONTENT
  std::vector<AqlValue> supervisedSlices;
  for (size_t i = 0; i < numRows; ++i) {
    supervisedSlices.emplace_back(content);
    block->setValue(i, 0, supervisedSlices.back());
  ...
  // Simulate toVelocyPack deduplication
  containers::FlatHashMap<AqlValue, size_t> table;
  size_t pos = 2;
  ...
  for (size_t row = 0; row < numRows; ++row) {
    ...
    if (inserted) {
      serializedCount++;  // Would serialize this value
      pos++;
    }
  }
  
  // Calculate memory usage
  size_t expectedSerializedSize = contentSize;  // Should serialize once
  size_t actualSerializedSize = serializedCount * contentSize;  // Actually serialized
  
  // EXPECTED: serializedCount = 1 (deduplicated)
  // CURRENT (OLD HASH): serializedCount = 10 (not deduplicated)
  EXPECT_EQ(1U, serializedCount)
      << "Expected: 1 serialization (deduplicated). "
      << "Current (old hash): " << serializedCount 
      << " serializations (not deduplicated). "
      << "Memory waste: " << (actualSerializedSize - expectedSerializedSize) << " bytes";
