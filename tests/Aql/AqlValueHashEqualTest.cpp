...
TEST_F(AqlValueHashEqualTest, inline_double_zero_variants) {
  ...
  // The current implementation uses VelocyPackHelper::equal() which uses comp<double>()
  // In C++, 0.0 == -0.0 is true, but the hash might differ based on bit representation
  ...
  // This is acceptable as hash collisions are allowed for equal values in some contexts
  bool areEqual = equal(val1, val2);
  if (areEqual) {
    // If equal, they should have same hash (hash/equality contract)
    EXPECT_EQ(hasher(val1), hasher(val2)) << "+0.0 and -0.0 are equal, so should have same hash";
  } else {
    // If not equal, that's also acceptable (IEEE 754 distinguishes them)
    // Just document this behavior
    EXPECT_FALSE(areEqual) << "+0.0 and -0.0 are treated as different (IEEE 754 behavior)";
  }
}
...
TEST_F(AqlValueHashEqualTest, real_world_deduplication_scenario) {
  ...
  // This uses std::unordered_set<AqlValue> which relies on std::hash<AqlValue> and std::equal_to<AqlValue>
  ...
  VPackBuilder builder;
  builder.add(VPackValue(42));
  AqlValue cloned(builder.slice());  // May be VPACK_INLINE or VPACK_MANAGED_SLICE
  
  sourceBlock->setValue(0, 0, original);
  sourceBlock->setValue(0, 1, cloned);  // Same semantic value, different storage
  
  // Call the actual cloneToBlock() method - this internally uses std::unordered_set<AqlValue>
  ...
  EXPECT_TRUE(equal(val0, val1)) << "Same semantic value should be deduplicated";
  EXPECT_EQ(hasher(val0), hasher(val1)) << "Same semantic value should have same hash";
}

TEST_F(AqlValueHashEqualTest, real_world_serialization_scenario) {
  // Test the actual AqlItemBlock::toVelocyPack() method
  // This uses FlatHashMap<AqlValue, size_t> which relies on std::hash<AqlValue> and std::equal_to<AqlValue>
  ...
  // Call the actual toVelocyPack() method - this internally uses FlatHashMap<AqlValue, size_t>
  velocypack::Builder result;
  result.openObject();
  block->toVelocyPack(0, block->numRows(), &velocypack::Options::Defaults, result);
  result.close();
  ...
  // This verifies that the hash/equality contract works correctly in the real implementation
}
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_deduplicates_values) {
  // Test actual AqlItemBlock::toVelocyPack() method - this uses FlatHashMap<AqlValue, size_t>
  ...
  // The raw array starts with 2 nulls (indices 0 and 1), then actual values start at index 2
  // With deduplication, same values should map to same positions
  size_t rawSize = raw.length();
  EXPECT_GE(rawSize, 2U) << "Raw array should have at least 2 nulls";
  EXPECT_LE(rawSize, 5U) << "Raw array should have at most 5 entries (2 nulls + 2-3 unique values)";
  
  // Verify we can deserialize and get back the same values
  // This tests that the hash/equality contract works correctly in the real implementation
}
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_handles_all_storage_types) {
  ...
  // 7 non-range values: inlineInt, inlineUInt, inlineDouble, sliceVal, stringVal, nullVal, boolVal
