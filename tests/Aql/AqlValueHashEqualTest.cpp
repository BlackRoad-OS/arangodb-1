...
TEST_F(AqlValueHashEqualTest, inline_double_zero_variants) {
  ...
  EXPECT_EQ(hasher(val1), hasher(val2))
      << "+0.0 and -0.0 should have same hash";
}
...
TEST_F(AqlValueToVelocyPackTest, to_velocypack_deduplicates_values) {
  ...
  EXPECT_NE(table.end(), it2)
      << "Same value in different storage should be found";
...
class AqlValueToVelocyPackDeduplicationTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    itemBlockManager = std::make_unique<AqlItemBlockManager>(*monitor);
  }
 ...
 *
 * EXPECTED BEHAVIOR:
 * - When serializing an AqlItemBlock with multiple rows containing supervised
 * slices that have the SAME CONTENT but DIFFERENT POINTERS, they should be
 * deduplicated.
 ...
 * - Hash uses pointer address → different supervised slices with same content
 * have different hashes → NOT deduplicated → wastes space
 *
...
TEST_F(AqlValueToVelocyPackDeduplicationTest,
  ...
  // Create a block with multiple rows containing supervised slices with SAME
  // CONTENT
  ...
  EXPECT_EQ(1U, uniqueCount)
      ...
      << "This test documents the EXPECTED behavior - it may fail with old "
         "hash.";
 ...
 *   that have the SAME CONTENT but DIFFERENT POINTERS, they should be
 * deduplicated.
 ...
 * CURRENT IMPLEMENTATION (OLD HASH - BUGGY):
 * - Hash uses pointer address → different supervised slices with same content
 * have different hashes → NOT deduplicated → wastes memory
 *
 * EXPECTED WITH NEW HASH:
 * - Hash uses content → same supervised slices with same content have same hash
...
TEST_F(AqlValueToVelocyPackDeduplicationTest,
  ...
  // All registers have supervised slices with SAME CONTENT but DIFFERENT
  // POINTERS
  constexpr RegisterId::value_t numCols = 5;
  ...
  SharedAqlItemBlockPtr clonedBlock =
      sourceRow.cloneToBlock(*itemBlockManager, allRegs, numCols);
  ...
  // EXPECTED: Should have only 1 unique data pointer (all registers reuse same
  // clone) CURRENT (OLD HASH): Will have 5 unique data pointers (different
  // pointers prevent deduplication) NEW HASH: Will have 1 unique data pointer
...
TEST_F(AqlValueToVelocyPackDeduplicationTest,
  ...
  size_t actualSerializedSize =
      serializedCount * contentSize;  // Actually serialized
  ...
  EXPECT_EQ(1U, serializedCount)
      ...
      << "Memory waste: " << (actualSerializedSize - expectedSerializedSize)
      << " bytes";
